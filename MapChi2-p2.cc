...
void MapChi2p2(string PID = "track") { //Segun la label, track, shower o ninguna de ellas, os hace esto para tracks, showers y middles

  TFile * f1 = new TFile("Events_1y_NO_Std_upto100.root"); //Aquí va el nombre de vuestro archivo, si no está en este path, dais el path entero
  auto hStdt = (TH3D * ) f1 -> Get("h_tracks_toy_all") -> Clone(""); //En los archivos que os mandaremos se llamará así el histograma en cos-theta/E
  auto hStds = (TH3D * ) f1 -> Get("h_showers_toy_all") -> Clone(""); //Data
  auto hStdm = (TH3D * ) f1 -> Get("h_middles_toy_all") -> Clone(""); //Data
  TFile * f2 = new TFile("Events_1y_NO_Std_upto100.root"); //El de arriba será para Data, el de abajo para Model
  ...
  if (PID == "track") {
    for (int i = 1; i <= nbinsx; i++) {
      for (int j = 1; j <= nbinsy; j++) {
        ...
        double std = hStdt -> GetBinContent(i, j, 1); //Este será para Data, ignorad los nombres de std, str, hStd, hNSI
        double str = hNSIt -> GetBinContent(i, j); //Este para Model
        cout << std << " " << str << endl;

        double sigma = 0;

        if (std > 0) sigma = (str - std) / sqrt(std); //Esta es la gaussiana
        if (std == 0) sigma = 0;
        if (sigma > 10 or sigma < -10) sigma = 0;
        ...
        sens += pow(sigma, 2); //Elevamos al cuadrado la sigma y sumamos para tener la total chi2
    ...
    for (int i = 1; i <= nbinsx; i++) {
      for (int j = 1; j <= nbinsy; j++) {
        ...
        double std = hStds -> GetBinContent(i, j, 1);
    ...
    for (int i = 1; i <= nbinsx; i++) {
      for (int j = 1; j <= nbinsy; j++) {
        ...
        double std = hStdm -> GetBinContent(i, j, 1);
